...
public class Main {
	...
	private static void test12(){
		...
		while(enJeu){
			...
			if(input != null){
				...
				System.out.println("Action non s�lectionnee");
			}
	...
	private static void test13(){ // IA à revoir, faute à minimax / eval pas tout à fait fonctionnel
		System.out.println("Test 13 - Jeu joueur contre machine correspondant à la question 6 du TP4");
		Game g = new Game();
		Board b = new Board(g, 5);
		int currentPlayer = 0; // le joueur 0 commencera la partie
		int mov = 0; // un petit compteur de mouvements
	...
	public static void test14(){ // boucle infinie (condition de victoire manquante) + IA à revoir (comme test 13)
		double debut = System.currentTimeMillis();	
		
		System.out.println("Test 13 - Jeu machine contre machine correspondant à la question 6 du TP4");
		Game g = new Game();
		Board b = new Board(g, 5);
		int currentPlayer = 0; // le joueur 0 commencera la partie
		int mov = 0; // un petit compteur de mouvements
 ...
 * 
 * 1) Dans ces cas, le premier joueur � poser une reine a forc�ment gagn�.
 * Le role du premier joueur determine forc�ment le vaincqueur (cf ci dessus)
 * Car si on force le premier joueur � poser un rocher, le deuxi�me va forc�ment poser une reine et gagner
 * 
 * 2) Elle semble juste dans le sens o� elle permet d'�quilibrer un peu le jeu en emp�chant le premier joueur de
 * prendre l'avantage en posant une reine � un endroit trop strategique, mais en l'autorisant tout de m�me � g�ner
 ...
 * 
 * 4)5) Selon moi, poser un rocher ou une reine ne devrait pas etre sujet a une diff�renciation de points. En effet, la strat�gie
 ...
 * 
 * 6) Les cases d�finitivement perdues sont celles directement adjacentes � une reine adverse. En effet, celles-ci sont soit sujettes �
 * l'action d'une reine, soit occup�es par un rocher pour neutraliser l'action de la dite reine sur la ligne en question. Toutes les autres
 * sont potentiellement r�cup�rables.
 * 
 * 
 * Reponse aux questions de la partie 3 du TP 4
 * 
 * le cas n°2 (avec QueenValue > RockValue (avec différence serrée) semble le plus approprié
*/